...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Event Banner */}
      <button type="button" aria-label="Open Booth Survey" className={`${isDark ? 'bg-teal-900/50 text-teal-200 border-white/5' : 'bg-teal-600 text-white border-transparent'} w-full border-b py-2.5 px-4 text-center text-sm font-medium flex items-center justify-center gap-4 group cursor-pointer hover:opacity-90 transition-all backdrop-blur-md`} onClick={() => setIsSurveyOpen(true)}>
      ...
      <nav className={`${isDark ? 'bg-[#05080a]/80 border-white/5' : 'bg-white/80 border-slate-200'} backdrop-blur-xl sticky top-0 z-50 border-b transition-colors duration-500`}>
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center h-20">
            {/* Logo Area */}
            <button type="button" aria-label="Go to Home" className="flex items-center gap-3 cursor-pointer" onClick={() => onNavigate(Page.HOME)}>
