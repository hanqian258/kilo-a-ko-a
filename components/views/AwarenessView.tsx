      ...
      {isEditorOpen && (
        <div className={`p-8 rounded-[2.5rem] shadow-2xl border mb-12 animate-in slide-in-from-top-4 transition-colors duration-500 ${isDark ? 'bg-[#0c1218] border-white/5' : 'bg-white border-slate-100'}`}>
          <div className="flex justify-between items-center mb-8">
            ...
            <button type="button" aria-label="Close editor" onClick={() => setIsEditorOpen(false)} className="text-slate-500 hover:text-teal-500"><X size={28} /></button>
          </div>
      ...
      {expandedArticleId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setExpandedArticleId(null)}>
          <div className={`w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative ${isDark ? 'bg-[#0c1218] border border-white/10 text-slate-300' : 'bg-white text-slate-600'}`} onClick={e => e.stopPropagation()}>
              <button type="button" aria-label="Close article" onClick={() => setExpandedArticleId(null)} className="absolute top-8 right-8 text-slate-500 hover:text-teal-500"><X size={28} /></button>
              {(() => {
            ...
            <div className="md:w-1/3 h-80 md:h-auto overflow-hidden relative">
              ...
              {canEdit && (
                <div className="absolute top-4 left-4 flex gap-2">
                  <button type="button" aria-label={`Edit ${article.title}`} title={`Edit ${article.title}`} onClick={() => handleEditClick(article)} className="bg-white/90 hover:bg-white p-2.5 rounded-xl shadow-lg text-teal-600 transition-all"><Edit2 size={16} /></button>
                  <button type="button" aria-label={`Delete ${article.title}`} title={`Delete ${article.title}`} onClick={() => handleDelete(article.id)} className="bg-white/90 hover:bg-white p-2.5 rounded-xl shadow-lg text-red-500 transition-all"><Trash2 size={16} /></button>
                </div>
              )}
            </div>
