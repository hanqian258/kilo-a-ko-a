                 ...
                 <div className="flex flex-wrap gap-4 items-center">
                    ...
                    {isAdmin && (
                      <div className="flex gap-2 ml-auto">
                         <button type="button" aria-label={`Edit ${event.title}`} title={`Edit ${event.title}`} onClick={() => handleEditClick(event)} className="bg-teal-500/10 hover:bg-teal-500/20 text-teal-500 p-2.5 rounded-xl transition-all"><Edit2 size={16} /></button>
                         <button type="button" aria-label={`Delete ${event.title}`} title={`Delete ${event.title}`} onClick={() => handleDeleteClick(event.id)} className="bg-red-500/10 hover:bg-red-500/20 text-red-500 p-2.5 rounded-xl transition-all"><Trash2 size={16} /></button>
                      </div>
                    )}
                 </div>
              </div>
